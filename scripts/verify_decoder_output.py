            ...
            for line_num, line in enumerate(f):
                ...
                try:
                    ...
                    if model_output is None:
                        print(
                            f"[WARN] Line {line_num}: No 'cleaned' or 'response' field found."
                        )
                        error_count += 1
                        continue
                    ...
                    if not isinstance(extracted_data, dict):
                        print(
                            f"[FAIL] Line {line_num}: Output parses but is not a Dictionary (Found {type(extracted_data)})."
                        )
                        error_count += 1
                    else:
                        valid_count += 1
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--input", type=str, required=True, help="Path to .jsonl output file"
    )
